...
pub const BUBBLEGUM_V1_PROGRAM_ID: Pubkey =
    solana_program::pubkey!("BGUMzZr2wWfD2yzrXFEWTK2HbdYhqQCP2EZoPEkZBD6o");
...
fn verify_bubblegum(bubblegum_program_account: &Pubkey) -> Result<()> {
    if bubblegum_program_account != &BUBBLEGUM_V1_PROGRAM_ID {
        msg!("found pubkey: {:?}", bubblegum_program_account);
        msg!("expected pubkey: {:?}", &BUBBLEGUM_V1_PROGRAM_ID);
        return Err(GumdropError::MustUseOfficialBubblegum.into());
    }
    Ok(())
}
...
pub mod gumdrop {
    ...
    pub fn new_distributor_compressed(
        ctx: Context<NewDistributorCompressed>,
        bump: u8,
        root: [u8; 32],
        temporal: Pubkey,
        max_depth: u32,
        ...
        msg!(&format!("initialized! temporal: {}", temporal));

        // Bubblegum create_tree CPI
        create_new_compressed_tree(
            distributor,
            &ctx.accounts.bubblegum_tree,
    ...
    pub fn claim_bubblegum<'info>(
        ctx: Context<'_, '_, '_, 'info, ClaimBubblegum<'info>>,
        claim_bump: u8,
        index: u64,
        amount: u64,
        claimant_secret: Pubkey,
        ...
        // TODO: this is a bit weird but we verify elsewhere that the bubblegum tree is
        // actually a valid tree
        // Verify the merkle proof.
        let node = solana_program::keccak::hashv(&[
            &[0x00],
            &index.to_le_bytes(),
            &claimant_secret.to_bytes(),
            &ctx.accounts.bubblegum_tree.key.to_bytes(),
            &amount.to_le_bytes(),
            &nft_metadata,
        ]);
        require!(
...
fn create_new_compressed_tree<'info>(
    distributor: &Account<'info, MerkleDistributor>,
    merkle_slab: &AccountInfo<'info>,
    merkle_slab_authority: &AccountInfo<'info>,
    ...
    let mut create_tree_data = vec![165, 83, 136, 142, 89, 202, 47, 220];
    msg!("{} {}", max_depth, max_buffer_size);
    create_tree_data.extend(max_depth.try_to_vec()?);
    create_tree_data.extend(max_buffer_size.try_to_vec()?);
    ...
    let account_metas = vec![
        AccountMeta::new(distributor.key(), true),
        AccountMeta::new_readonly(merkle_slab_authority.key(), false),
        AccountMeta::new_readonly(gummyroll_program.key(), false),
        AccountMeta::new(merkle_slab.key(), false),
    ];
    let bubblegum_infos = vec![
        // tree creator
        distributor.to_account_info(),
        // authority
        merkle_slab_authority.to_account_info(),
        // gummyroll program
    ...
    invoke_signed(
        &Instruction {
            ...
            // TODO. global::bubblegum::create_tree instruction...
            data: create_tree_data,
        },
        &bubblegum_infos,
        &[&distributor_seeds],
    )?;
...
fn issue_compressed_nft<'info>(
    ...
    //  Actually issue the compressed NFT
    //  --------------------------------
    ...
    let account_metas = vec![
        // mint authority == distributor
        AccountMeta::new_readonly(distributor.key(), true),
        // merkle tree owner (bubblegum auth)
        AccountMeta::new_readonly(merkle_tree_auth.key(), false),
        // nonce (writable)
        AccountMeta::new(nonce.key(), false),
        // gummyroll
        AccountMeta::new_readonly(gummyroll_program.key(), false),
        // owner (has to be a signer)
        AccountMeta::new_readonly(payer.key(), true),
        // delegate
        AccountMeta::new_readonly(payer.key(), false),
        // merkle slab (writable)
        AccountMeta::new(merkle_slab.key(), false),
    ];
    let bubblegum_infos = vec![
        // mint authority == distributor
    ...
    msg!("Invoking bubblegum to mint an airdropped NFT");
    invoke_signed(
        &Instruction {
            program_id: bubblegum_program.key(),
            accounts: account_metas,
            // TODO. global::bubblegum::mint_nft instruction...
            data: ix_data,
        },
        &bubblegum_infos,
        &[&distributor_seeds],
...
#[derive(Accounts)]
#[instruction(bump: u8)]
pub struct NewDistributorCompressed<'info> {
    /// Base key of the distributor.
    pub base: Signer<'info>,
    ...
    /// Bubblegum-tree. Verified through CPI
    #[account(mut)]
    /// CHECK: Checked via CPI
    pub bubblegum_tree: AccountInfo<'info>,

    /// CHECK: Checked via CPI
    pub bubblegum_tree_authority: AccountInfo<'info>,
    ...
    /// [Gummyroll] program.
    /// CHECK: account checked in handler
    pub gummyroll_program: AccountInfo<'info>,
...
// TODO(ngundotra): Add a bubblegum-level wallet
/// [gumdrop::claim_bubblegum_proven] accounts.
#[derive(Accounts)]
#[instruction(claim_bump: u8, index: u64)]
pub struct ClaimBubblegum<'info> {
    /// The [MerkleDistributor].
    #[account(mut)]
    pub distributor: Account<'info, MerkleDistributor>,
...
pub enum GumdropError {
    ...
    #[msg("Gumdrop only supports the official Metaplex Bubblegum contracts")]
    MustUseOfficialBubblegum,
}
