...
async function main() {
  ...
  await truncateWithGumball(connection, provider, payer, GumballMachine);
}
...
async function truncateViaBubblegum(
  ...
  if (!acctInfo || acctInfo.lamports === 0) {
    ...
    console.log(
      "Requesting airdrop:",
      await connection.requestAirdrop(payer.publicKey, 5e10)
    );
    createIxs = await getCreateTreeIxs(
      connection,
      MAX_DEPTH,
      MAX_BUFFER_SIZE,
      CANOPY_DEPTH,
    ...
    console.log(
      "Bubblegum tree already exists:",
      bgumTree.publicKey.toBase58()
    );
  }
...
async function getTransferInfoFromServer(
  leafHash: Buffer,
  treeId: PublicKey
): Promise<ProofResult> {
...
async function testWithBubblegumTransfers(
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const { root, dataHash, creatorHash, proofNodes, nonce, index } =
      await getTransferInfoFromServer(leafHash, bgumTree.publicKey);
    const transferIx = addProof(
      createTransferInstruction(
        {
          authority,
          candyWrapper: CANDY_WRAPPER_PROGRAM_ID,
    ...
    txs.push(
      await connection.getTransaction(txIds[txIds.length - 1], {
        commitment: "confirmed",
      })
    );
  }
  console.log(
    `Transferred all NFTs to ${finalDestination.publicKey.toString()}`
  );
...
async function initializeGumballMachine(
  ...
  await initializeGumballMachineIndices(
    gumballMachine.provider,
    gumballMachineInitArgs.maxItems,
    authority,
    gumballMachineAcctKeypair.publicKey
  );
}
...
async function addConfigLines(
  ...
  await execute(provider, [addConfigLinesInstr], [authority]);
}
...
async function truncateWithGumball(
  ...
  await connection.requestAirdrop(payer.publicKey, 11 * LAMPORTS_PER_SOL);
  await connection.requestAirdrop(nftBuyer.publicKey, 11 * LAMPORTS_PER_SOL);
  ...
  const MERKLE_ROLL_ACCT_SIZE = getMerkleRollAccountSize(
    baseGumballMachineInitProps.maxDepth,
    baseGumballMachineInitProps.maxBufferSize
  );

  if (
    !(await connection.getAccountInfo(merkleRollKeypair.publicKey, "confirmed"))
  ) {
